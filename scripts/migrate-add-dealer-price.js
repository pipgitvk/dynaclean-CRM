// Database migration script for adding dealer_price field to products_list
const fs = require("fs");
...
async function runMigration() {
  ...
  try {
    ...
    console.log("\n📋 Checking dealer_price column...");
    try {
      await connection.execute(
        `ALTER TABLE products_list ADD COLUMN dealer_price DECIMAL(10, 2) DEFAULT 0 AFTER gem_price`
      );
      console.log("✅ dealer_price column added successfully");
    } catch (err) {
      if (err.code === 'ER_DUP_FIELDNAME') {
        console.log("✅ dealer_price column already exists");
      } else {
        throw err;
      }
    }
