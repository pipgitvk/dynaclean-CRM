...
class RecurrenceService {
  ...
  async ensureRecurringTasksTable(conn) {
    await conn.execute(`
  ...
  async ensureParentRecurringTaskColumn(conn) {
    try {
      await conn.execute(
        `ALTER TABLE task ADD COLUMN parent_recurring_task_id INT NULL`
      );
      console.log("✅ Added task.parent_recurring_task_id column");
    } catch (error) {
    ...
    try {
      ...
      if (error.errno !== 1061 && error.errno !== 1060) throw error;
    }
  }
  ...
  /** Run once per request: recurring_tasks table + task columns for cron/UI. */
  async ensureRecurringSchema(conn) {
    await this.ensureRecurringTasksTable(conn);
    await this.ensureParentRecurringTaskColumn(conn);
  ...
  async backfillAutomaticTaskAssignDates(conn) {
    try {
      await this.ensureRecurringSchema(conn);
