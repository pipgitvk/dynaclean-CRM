...
class NotificationService {
  async resolveAutomaticTaskRecipients(conn, recurringTask, taskMeta = {}) {
    const ids = new Set();
    const add = (id) => {
      const n = Number(id);
      if (n > 0) ids.add(n);
    ...
    if (taskMeta.assigneeUsername) {
      add(await resolveGemCrmEmployeeId(conn, { username: taskMeta.assigneeUsername }));
    }
    if (taskMeta.creatorUsername) {
      add(await resolveGemCrmEmployeeId(conn, { username: taskMeta.creatorUsername }));
    }
  ...
  async sendReassignNotification(
    ...
    try {
      ...
      const newAssigneeId = await resolveGemCrmEmployeeId(conn, {
