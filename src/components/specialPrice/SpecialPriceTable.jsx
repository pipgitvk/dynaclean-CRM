...
export default function SpecialPriceTable({
  rows = [],
  currentPage,
  totalPages,
  userRole,
            ...
            rows.map((row) => (
              <tr key={row.id} className="border-t">
                ...
                <td className="p-3 flex gap-2">
                  <Link
                    href={`/user-dashboard/special-pricing/${row.customer_id}`}
                    className="text-green-600 hover:underline"
                  >
                    View
                  </Link>
                  <Link
                    href={`/user-dashboard/special-pricing/${row.customer_id}/${row.product_id}`}
