...
//   const [statusFilter, setStatusFilter] = useState(null); // "IN" or "OUT" or null
...
//       { header: "Item Name", key: "item_name", width: 25 }, // Added Item Name
//       { header: "Product Number", key: "product_number", width: 20 }, // Added Product Number
...
//         [
//           "Product Code",
//           "Item Name", // Added Item Name
//           "Product Number", // Added Product Number
//           "Latest Qty",
//           "Total Qty",
//           "From Company",
//           "Location",
...
//         row.product_code,
//         row.item_name, // Added Item Name
//         row.product_number, // Added Product Number
//         row.quantity,
//         row.total_quantity,
//         row.from_company,
//         row.location,
...
//       <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto relative">
...
//                 {[
//                   "product_code",
//                   "product_number", // Added new column
//                   "item_name", // Added new column
//                   // "quantity",
//                   "total_quantity",
//                   "from_company",
//                   "location",
...
export default function StockStatusModal({ isOpen, onClose }) {
  ...
  function handleDoubleClick(productCode, key, value) {
    setEditingCell({ id: productCode, key });
    setEditValue(value);
  }

  // Handle saving the edited value
  async function handleSave(productCode) {
    if (editingCell.key === "min_qty" && editValue !== "") {
      try {
        const response = await fetch("/api/product/update-min-qty", {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          setData(updatedData);
          console.log(
            `Min Qty for Product Code ${productCode} updated successfully.`
          );
        } else {
  ...
  async function exportToXLS() {
    ...
    worksheet.columns = [
      ...
      { header: "Product Number", key: "product_number", width: 20 },
      { header: "Latest Qty", key: "quantity", width: 15 },
      { header: "Total Qty", key: "total_quantity", width: 15 },
      { header: "Min Qty", key: "min_qty", width: 15 }, // Added min_qty
      { header: "From Company", key: "from_company", width: 20 },
      { header: "Location", key: "location", width: 15 },
      { header: "Added By", key: "added_by", width: 20 },
      { header: "Sold To", key: "to_company", width: 20 },
            ...
            <tbody>
              {filteredData.map((row, idx) => (
                <tr key={row.product_code + "_" + idx} className="border-t">
                  ...
                  >
                    {editingCell.id === row.product_code &&
                    editingCell.key === "min_qty" ? (
                      <input
                        type="number"
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => handleSave(row.product_code)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            handleSave(row.product_code);
                          }
                        }}
