...
import { History, Eye, X, ClipboardList } from "lucide-react";
...
export function FollowUpModal({ fu, onClose, onSaved, allowEditIdentity = false }) {
  ...
  useEffect(() => {
    if (!allowEditIdentity || serialSearch.length < 2) {
      setSuggestions([]);
      setShowSugg(false);
      return;
    }
  ...
  }, [serialSearch, allowEditIdentity]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              readOnly={!allowEditIdentity}
              disabled={!allowEditIdentity}
              onChange={allowEditIdentity ? set("product_model") : undefined}
          ...
          <div>
            ...
            <input
              ...
              readOnly={!allowEditIdentity}
              disabled={!allowEditIdentity}
              onChange={allowEditIdentity ? set("contact") : undefined}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-transparent">
      <div className="relative">
        ...
        <img src={image} alt="Follow-up preview" className="max-w-[90vw] max-h-[90vh] object-contain rounded-lg" />
      </div>
    </div>
  );
}

export function FollowUpActionButtons({ onFollowUp, onHistory, className = "", variant = "button" }) {
  if (variant === "icon") {
    return (
      <div className={`flex items-center gap-1 ${className}`}>
        <button
          ...
          className="inline-flex items-center justify-center p-1.5 rounded-md bg-green-600 hover:bg-green-700 text-white transition-colors"
        >
        ...
        <button
          ...
          className="inline-flex items-center justify-center p-1.5 rounded-md bg-purple-600 hover:bg-purple-700 text-white transition-colors"
        >
          <History size={14} />
        </button>
      </div>
  ...
  return (
    <div className={`flex flex-wrap gap-1 mt-2 ${className}`}>
      <button
        onClick={onFollowUp}
        className="flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded text-xs whitespace-nowrap"
      >
        <ClipboardList size={12} /> Follow Up
      </button>
      <button
        onClick={onHistory}
        className="flex items-center gap-1 bg-purple-600 hover:bg-purple-700 text-white px-2 py-1 rounded text-xs whitespace-nowrap"
      >
