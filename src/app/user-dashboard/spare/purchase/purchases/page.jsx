...
function SpareEditTransportModal({ open, onClose, record, onSaved }) {
  ...
  async function save() {
    try {
      ...
      if (mode === "Courier") {
        fd.append("courier_tracking_id", form.courier_tracking_id);
        fd.append("courier_company", form.courier_company);
      }
      if (mode === "Porter") {
        fd.append("porter_tracking_id", form.porter_tracking_id);
        fd.append("porter_contact", form.porter_contact);
      }
      if (mode === "Truck") {
        fd.append("truck_number", form.truck_number);
        fd.append("driver_name", form.driver_name);
        fd.append("driver_number", form.driver_number);
      }
      if (files.quotation_upload)
        fd.append("quotation_upload", files.quotation_upload);
      if (files.payment_proof_upload)
        fd.append("payment_proof_upload", files.payment_proof_upload);
      if (files.invoice_upload)
        fd.append("invoice_upload", files.invoice_upload);
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl w-full max-w-xl p-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold">
            Edit Transport (Req #{record?.id})
          </h3>
          ...
          {mode === "Self" && (
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, self_name: e.target.value })
                }
                disabled={disabled}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, courier_tracking_id: e.target.value })
                  }
                  disabled={disabled}
                />
              </div>
          ...
          {mode === "Porter" && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  className="w-full border p-2 rounded"
                  value={form.porter_tracking_id}
                  onChange={(e) =>
                    setForm({ ...form, porter_tracking_id: e.target.value })
                  }
                  disabled={disabled}
                />
              </div>
              <div>
                ...
                <input
                  className="w-full border p-2 rounded"
                  value={form.porter_contact}
                  onChange={(e) =>
                    setForm({ ...form, porter_contact: e.target.value })
                  }
                  disabled={disabled}
                />
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, driver_number: e.target.value })
                  }
                  disabled={disabled}
                />
              </div>
              ...
              <div>
                ...
                {record?.quotation_upload && (
                  <div className="text-xs text-gray-600 mb-1 flex items-center gap-1">
                    Current:{" "}
                    <a
                      href={`/${String(record.quotation_upload).replace(/^\/+/, "")}`}
                      target="_blank"
                      rel="noreferrer"
                ...
                <input
                  ...
                  onChange={(e) => onFileChange(e, "quotation_upload")}
                  className="w-full border p-2 rounded"
                />
              </div>
              <div>
                ...
                {record?.payment_proof_upload && (
                  <div className="text-xs text-gray-600 mb-1 flex items-center gap-1">
                    Current:{" "}
                    <a
                      href={`/${String(record.payment_proof_upload).replace(/^\/+/, "")}`}
                      target="_blank"
                      rel="noreferrer"
                      className="text-blue-600 underline flex items-center gap-1"
                    >
                ...
                <input
                  type="file"
                  accept=".pdf,.doc,.docx,image/*"
                  onChange={(e) => onFileChange(e, "payment_proof_upload")}
                  className="w-full border p-2 rounded"
                />
              </div>
              <div>
                ...
                {record?.invoice_upload && (
                  <div className="text-xs text-gray-600 mb-1 flex items-center gap-1">
                    Current:{" "}
                    <a
                      href={`/${String(record.invoice_upload).replace(/^\/+/, "")}`}
                      target="_blank"
                      rel="noreferrer"
                      className="text-blue-600 underline flex items-center gap-1"
                    >
                ...
                <input
                  type="file"
                  accept=".pdf,.doc,.docx,image/*"
                  onChange={(e) => onFileChange(e, "invoice_upload")}
                  className="w-full border p-2 rounded"
                />
              </div>
              <div>
                ...
                {record?.eway_bill && (
                  <div className="text-xs text-gray-600 mb-1 flex items-center gap-1">
                    Current:{" "}
                    <a
                      href={`/${String(record.eway_bill).replace(/^\/+/, "")}`}
                      target="_blank"
                      rel="noreferrer"
                      className="text-blue-600 underline flex items-center gap-1"
                    >
                ...
                <input
                  type="file"
                  accept=".pdf,.doc,.docx,image/*"
                  onChange={(e) => onFileChange(e, "eway_bill")}
                  className="w-full border p-2 rounded"
                />
              </div>
            </div>
...
function LinkPaymentModal({
  ...
  const eligibleStatements = useMemo(() => {
    ...
    let rows = statements.filter((s) => {
      ...
      if (
        Array.isArray(currentStatementIds) &&
        currentStatementIds.includes(Number(s?.id))
      )
        return true;
    ...
    if (q) {
      rows = rows.filter((s) => {
        ...
        const matchesPurchaseId =
          isNumericSearch &&
          (linked.includes(`ps${qRaw}`) || linked.includes(`pp${qRaw}`));
    ...
    try {
      ...
      if (action === "link") {
        if (currentTotal + stmtAmount > Number(purchase.net_amount || 0)) {
          toast.error(
            `Cannot link: Total payment (₹${(currentTotal + stmtAmount).toLocaleString()}) would exceed Net Amount (₹${Number(purchase.net_amount || 0).toLocaleString()})`,
          );
          return;
        }
      }
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl w-full max-w-5xl p-4 max-h-[85vh] overflow-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="flex flex-col gap-0.5 mt-0.5">
              ...
              <p className="text-xs font-medium">
                ...
                <span className="text-blue-700">
                  ₹{Number(purchase?.net_amount || 0).toLocaleString()}
                </span>
                <span className="mx-2 text-gray-300">|</span>
                <span className="text-gray-600">Total Linked:</span>{" "}
                <span
                  className={`${currentTotal > Number(purchase?.net_amount || 0) ? "text-red-600" : "text-emerald-700"}`}
                >
                ...
                <span className="text-gray-800">
                  ...
                  {Math.max(
                    0,
                    Number(purchase?.net_amount || 0) - currentTotal,
                  ).toLocaleString()}
                </span>
              </p>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <div className="text-xs text-gray-600 font-medium">
            {loading
              ? "Loading..."
              : `Showing ${eligibleStatements.length} statement(s)`}
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                eligibleStatements.map((s) => {
                  ...
                  if (isLinking)
                    btnClass += "bg-gray-200 text-gray-500 cursor-wait";
                  else if (alreadyLinkedToThis)
                    btnClass +=
                  ...
                  return (
                    <tr
                      key={s.id}
                      className={`hover:bg-gray-50 transition-colors ${alreadyLinkedToThis ? "bg-emerald-50/30" : ""}`}
                    >
                      ...
                      <td
                        className="p-3 max-w-[400px] truncate text-gray-600"
                        title={s.description || ""}
                      >
                        {s.description || "—"}
                      </td>
...
export default function SparePurchasesPage() {
  ...
  const [paymentStatementByPurchaseId, setPaymentStatementByPurchaseId] =
    useState(() => ({}));
    ...
    if (action === "link") {
      ...
      setPaymentTransByPurchaseId((prev) => {
        const current = prev?.[pid]
          ? String(prev[pid]).split(", ").filter(Boolean)
          : [];
      ...
      // unlink
      setPaymentStatementByPurchaseId((prev) => {
        const current = Array.isArray(prev?.[pid])
          ? prev[pid].filter((id) => id !== Number(statementId))
          : [];
        if (current.length === 0) {
  ...
  const filteredPurchases = useMemo(() => {
    ...
    if (search) {
      ...
      filtered = filtered.filter((p) => {
        ...
        const transIds = String(
          paymentTransByPurchaseId?.[Number(p.id)] || "",
        ).toLowerCase();
    ...
    if (endDate) {
      filtered = filtered.filter(
        (p) => new Date(p.created_at) <= new Date(endDate + "T23:59:59"),
      );
    }
    // Sorting
    filtered = [...filtered].sort((a, b) => {
      ...
      if (
        ["quantity", "price_per_unit", "net_amount", "id"].includes(sortColumn)
      ) {
        aVal = Number(aVal) || 0;
        bVal = Number(bVal) || 0;
      }
    ...
    Object.values(parentMap)
      ...
      .forEach((parent) => {
        ...
        if (childMap[parent.id]) {
          sortedFiltered.push(
            ...childMap[parent.id].sort((a, b) => a.id - b.id),
          );
        }
      });
    ...
    setSelectedPurchaseIds((prev) => {
      ...
      if (newSet.has(id)) {
        ...
        if (purchase?.parent_id) {
          ...
          purchases
            .filter((p) => p.parent_id === purchase.parent_id)
            .forEach((child) => newSet.delete(child.id));
        }
      } else {
        newSet.add(id);
  ...
  const toggleSelectAllPurchases = () => {
    if (selectedPurchaseIds.size === filteredPurchases.length) {
      ...
      setSelectedPurchaseIds(
        new Set(filteredPurchases.map((p) => Number(p.id))),
      );
    }
  };
  ...
  async function exportToXLS() {
    ...
    a.href = url;
    a.download = "spare_purchases.xlsx";
    a.click();
  ...
  function exportToPDF() {
    ...
    autoTable(doc, {
      ...
      body: filteredPurchases.map((row) => [
        row.id,
        row.spare_name,
        row.quantity,
        row.net_amount,
    ...
    <div className="max-w-full mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          All Spare Purchase Requests
        </h1>
        <p className="text-gray-600 mt-1">
          View and manage spare stock purchase requests
        </p>
      </div>
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded ${statusFilter === key ? `${activeBg} text-white` : "bg-gray-200 text-gray-800 hover:bg-gray-300"}`}
                >
                  {label}
                </button>
              ))}
        ...
        <div className="flex flex-wrap gap-4 p-3 bg-blue-50 rounded-md border border-blue-200">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-purple-700">
              ₹
              {totals.totalPricePerUnit.toLocaleString("en-IN", {
                minimumFractionDigits: 2,
          ...
          {selectedPurchaseIds.size > 0 && (
            <div className="flex items-center gap-2 ml-auto">
              ...
              <span className="text-sm font-bold text-orange-700">
                ₹
                {selectedTotals.selectedNetAmount.toLocaleString("en-IN", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </span>
            ...
            <table className="min-w-full text-sm">
                ...
                <tr>
                  <th className="p-3 border-b font-semibold">
                    <input
                      type="checkbox"
                      checked={
                        selectedPurchaseIds.size === filteredPurchases.length &&
                        filteredPurchases.length > 0
                      }
                  ...
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
                  ...
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
                    onClick={() => handleSort("spare_name")}
                  >
                    Spare Name{" "}
                    {sortColumn === "spare_name" &&
                      (sortDirection === "asc" ? "↑" : "↓")}
                  </th>
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
                  ...
                  <th className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200">
                    Paid Amount
                  </th>
                  <th className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200">
                    Remaining Amount
                  ...
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
                    onClick={() => handleSort("from_company")}
                  >
                    From Company{" "}
                  ...
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
                    onClick={() => handleSort("mode_of_transport")}
                  >
                    Transport{" "}
                    {sortColumn === "mode_of_transport" &&
                      (sortDirection === "asc" ? "↑" : "↓")}
                  </th>
                  <th
                    className="p-3 border-b font-semibold cursor-pointer hover:bg-gray-200"
              ...
              <tbody>
                {filteredPurchases.map((purchase) => {
                  ...
                  const isParent = purchases.some(
                    (p) => p.parent_id === purchase.id,
                  );

                  return (
                    <tr
                      key={purchase.id}
                      className={`border-t hover:bg-gray-50 ${isChild ? "bg-gray-50" : ""}`}
                    >
                      <td className="p-3">
                        <input
                          type="checkbox"
                          ...
                          onChange={() =>
                            togglePurchaseSelection(Number(purchase.id))
                          }
                          className="w-4 h-4 text-blue-600 rounded"
                        />
                      </td>
                        ...
                        <div className="flex items-center gap-2">
                          {isChild && (
                            <span className="text-gray-400 text-xs pl-4">
                              ↳ Child of #{purchase.parent_id}
                            </span>
                          )}
                          <span
                            className={`${isChild ? "text-gray-500" : "font-semibold"}`}
                          >
                            #{purchase.id}
                          </span>
                          {isParent && (
                      ...
                      <td className="p-3">
                        ₹{Number(purchase.price_per_unit || 0).toFixed(2)}
                      </td>
                      <td className="p-3 font-semibold">
                        ₹{Number(purchase.net_amount || 0).toFixed(2)}
                      </td>
                      ...
                      <td className="p-3 text-center">
                        {purchase.spare_image ? (
                          <button
                            onClick={() =>
                              setPreviewImage({
                                url: `/${String(purchase.spare_image).replace(/^\/+/, "")}`,
                                type: getFileType(purchase.spare_image),
                              })
                            }
                      ...
                      <td className="p-3 font-mono text-xs">
                        {paymentTransByPurchaseId?.[Number(purchase.id)]
                          ? paymentTransByPurchaseId[Number(purchase.id)]
                          : "—"}
                      </td>
                      <td className="p-3">
                        <div className="flex flex-col gap-1">
                          {isChild ? (
                            <span className="text-xs text-gray-500">
                              Managed via Parent #{purchase.parent_id}
                            </span>
                          ) : isParent ? (
                            <span className="text-xs text-gray-500">
                              Manage via Multi-Select
                            </span>
      ...
      <LinkPaymentModal
        ...
        currentStatementIds={
          paymentStatementByPurchaseId?.[Number(linkPurchase?.id)]
        }
      />
      ...
      {detailPurchase && (
        ...
        >
          <div
            className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[85vh] overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b">
              <h3 className="text-lg font-semibold">
                Request #{detailPurchase.id} Details
              </h3>
              <button
            ...
            <div className="p-4">
              {(() => {
                const stages = ["requested", "in_warehouse", "fulfilled"];
                const idx = Math.max(0, stages.indexOf(detailPurchase.status));
                ...
                return (
                  <div>
                    ...
                    <div className="w-full h-2 rounded bg-gray-200">
                      <div
                        className={`h-2 rounded ${idx === 0 ? "bg-yellow-500" : idx === 1 ? "bg-blue-500" : "bg-green-600"}`}
                        style={{ width: percent + "%" }}
                      />
                    </div>
                  </div>
            ...
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                [
                  "Price/Unit",
                  `₹${Number(detailPurchase.price_per_unit || 0).toFixed(2)}`,
                ],
                ["Amount/Unit", detailPurchase.amount_per_unit],
                [
                  "Net Amount",
                  `₹${Number(detailPurchase.net_amount || 0).toFixed(2)}`,
                ],
                ...
                [
                  "Received Date",
                  detailPurchase.received_date
                    ? new Date(detailPurchase.received_date).toLocaleString()
                    : "",
                ],
      ...
      {previewImage && (
        ...
        >
          ...
          >
            ...
            {(() => {
              const type = getFileType(previewImage.url);
              return type === "image" ? (
                <img
