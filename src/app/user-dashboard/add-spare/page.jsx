...
export default function AddSparePage() {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    return (
      ...
      String(p.item_code || "").toLowerCase().includes(query) ||
      String(p.product_number || "").toLowerCase().includes(query)
    );
  });
  ...
  const removeMachine = (machineToRemove) => {
    setFormData((prev) => ({
      ...prev,
      compatible_machine: prev.compatible_machine.filter((m) => m !== machineToRemove),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("compatible_machine", formData.compatible_machine.join(","));
    ...
    try {
      const response = await fetch("/api/spare/add", { method: "POST", body: data });
      if (response.ok) {
  ...
  const handleSearchContinue = () => {
    ...
    if (!name) { setSearchError("Please enter a spare name"); return; }
    const exists = spareIndex.some(
      (s) => String(s.item_name).toLowerCase() === name.toLowerCase()
    );
    if (exists) { setSearchError("Spare already exists. Please use a different name."); return; }
  ...
  return (
    <div className="bg-gray-50 flex flex-col items-center justify-center p-4 md:p-10">
      <Head><title>Add New Spare Part</title></Head>
      ...
      {showSearchModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl p-6">
            ...
            {!loadedIndex && <SpareIndexLoader setLoadedIndex={setLoadedIndex} setSpareIndex={setSpareIndex} />}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-3 justify-start">
                  <button type="button" onClick={() => router.back()} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Cancel</button>
                  <button type="button" onClick={handleSearchContinue} className="px-4 py-2 rounded-lg text-white font-semibold bg-blue-600 hover:bg-blue-700">Continue</button>
                </div>
              </div>
              <div className="max-h-64 overflow-auto border rounded-lg p-2 bg-gray-50">
                ...
                <ul className="divide-y">
                  {spareIndex
                    .filter((s) => !searchInput || s.item_name?.toLowerCase().includes(searchInput.toLowerCase()) || s.specification?.toLowerCase().includes(searchInput.toLowerCase()))
                    .slice(0, 20)
                    .map((s, idx) => (
                  ...
                  {loadedIndex && spareIndex.length === 0 && <li className="py-2 text-sm text-gray-500">No spares found.</li>}
                </ul>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white shadow-xl rounded-2xl p-8 w-full max-w-4xl">
        <div className="flex items-center justify-between mb-8">
          <button type="button" onClick={() => router.back()} className="flex items-center text-blue-600 hover:text-blue-800 transition-colors duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
            </svg>
            Back
          </button>
          <h1 className="text-3xl font-extrabold text-gray-800 text-center flex-grow">Add New Spare Part</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="flex flex-col">
              <label htmlFor="item_name" className="text-sm font-semibold text-gray-700 mb-2">Item Name <span className="text-red-500">*</span></label>
              <input id="item_name" name="item_name" type="text" value={formData.item_name} onChange={handleChange}
                placeholder="e.g., Spark Plug"
                className={`p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${showSearchModal ? "bg-gray-100" : ""}`}
                required readOnly={showSearchModal} />
            </div>

            {/* Specification */}
            <div className="flex flex-col">
              <label htmlFor="specification" className="text-sm font-semibold text-gray-700 mb-2">Specification</label>
              <input id="specification" name="specification" type="text" value={formData.specification} onChange={handleChange}
            ...
            {/* Make */}
            <div className="flex flex-col">
              <label htmlFor="make" className="text-sm font-semibold text-gray-700 mb-2">Make</label>
            ...
            {/* Model */}
            <div className="flex flex-col">
              <label htmlFor="model" className="text-sm font-semibold text-gray-700 mb-2">Model</label>
                ...
                <>
                      ...
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-64 overflow-y-auto z-50">
                        {filteredProducts.length > 0 ? (
                          filteredProducts.slice(0, 20).map((product) => (
                            <button key={product.id} type="button" onClick={() => addMachine(product)}
                              ...
                              <div className="flex-1">
                                <div className="font-semibold text-gray-800">{product.item_name}</div>
                                <div className="text-sm text-gray-500">Product #: {product.product_number}</div>
                              </div>
                              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full ml-2">Add</span>
                            </button>
                          ))
                        ) : (
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {formData.compatible_machine.map((machine, index) => {
                      ...
                      return (
                        <span key={index} className="flex items-center px-3 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                          {displayCode}
                          <button type="button" onClick={() => removeMachine(machine)} className="ml-2 text-blue-500 hover:text-blue-700 font-bold">✕</button>
                        </span>
                      );
                    })}
                  </div>
                  {formData.compatible_machine.length === 0 && <p className="mt-2 text-xs text-gray-500">No machines selected yet</p>}
                </>
              ) : (
                <>
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && e.target.value.trim()) {
                        ...
                        if (!formData.compatible_machine.includes(val)) {
                          setFormData(prev => ({ ...prev, compatible_machine: [...prev.compatible_machine, val] }));
                        }
                        e.target.value = "";
                      }
                    }}
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {formData.compatible_machine.map((machine, index) => (
                      <span key={index} className="flex items-center px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                        {machine}
                        <button type="button" onClick={() => removeMachine(machine)} className="ml-2 text-blue-500 hover:text-blue-700">&times;</button>
                      </span>
                    ))}
                  </div>
                </>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 mt-4 p-4 border rounded-lg bg-gray-50">
            <div className="flex flex-col">
              <label htmlFor="image" className="text-sm font-semibold text-gray-700 mb-2">Image (Optional)</label>
              <input id="image" name="image" type="file" onChange={(e) => setImageFile(e.target.files[0])}
                className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" />
            </div>
            <div className="flex flex-col">
              <label htmlFor="catalog" className="text-sm font-semibold text-gray-700 mb-2">Catalog (Optional, PDF)</label>
              <input id="catalog" name="catalog" type="file" accept=".pdf" onChange={(e) => setCatalogFile(e.target.files[0])}
                className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" />
            </div>
          </div>
          ...
          <div className="mt-8">
            <button type="submit"
              className={`w-full py-4 px-6 rounded-lg text-white font-bold text-lg transition-colors duration-200 ${loading || showSearchModal ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}
