...
export default function EmployeeLeadsTable() {
    ...
    const fetchLeads = async (campaign) => {
        try {
            setLoading(true);
            const url = campaign ? `/api/employee-leads?campaign=${campaign}` : "/api/employee-leads";
    ...
    const uniqueCampaigns = [...new Set(leads.map(lead => lead.lead_campaign).filter(Boolean))];
    const uniqueCreators = [...new Set(leads.map(lead => lead.created_by).filter(Boolean))];
    ...
    const filteredLeads = leads.filter((lead) => {
        ...
        if (filterStage === "all") {
            ...
            matchesStage = lead.current_stage === "Quotation Created" || lead.current_stage === "Order Created" || lead.current_stage === "Order Processed";
        } else if (filterStage === "Order Created") {
            matchesStage = lead.current_stage === "Order Created" || lead.current_stage === "Order Processed";
        } else {
            matchesStage = lead.current_stage === filterStage;
        }

        const matchesCampaign = filterCampaign === "all" || lead.lead_campaign === filterCampaign;
        const matchesCreatedBy = filterCreatedBy === "all" || lead.created_by === filterCreatedBy;
        ...
        if (startDate || endDate) {
            ...
            if (!leadDate) {
                ...
                const endDateObj = endDate ? new Date(new Date(endDate).getTime() + 24 * 60 * 60 * 1000) : null; // endDate inclusive
        ...
        return matchesSearch && matchesStage && matchesCampaign && matchesCreatedBy && matchesDate;
    });

    // Calculate KPIs based on filtered leads (respecting date range & other filters)
    const uniqueCustomers = [...new Set(filteredLeads.map(lead => lead.customer_id))];
    const totalLeads = uniqueCustomers.length;
    const quotationsCreated = filteredLeads.filter(lead => lead.quote_number).length;
    const ordersCreated = filteredLeads.filter(lead => lead.order_id && lead.account_status === 1).length;
    const ordersProcessed = filteredLeads.filter(lead => lead.order_id && lead.account_status === 1 && lead.dispatch_status === 1).length;
    const conversionRatio = totalLeads > 0 ? (ordersCreated / totalLeads) * 100 : 0;
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <button className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left" onClick={() => setFilterStage("all")}>
                    <p className="text-sm text-gray-600">Total Leads</p>
                    <p className="text-2xl font-bold text-blue-700">{totalLeads}</p>
                </button>
                <button className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-left" onClick={() => setFilterStage("Quotation Created")}>
                    <p className="text-sm text-gray-600">Quotations Created</p>
                    <p className="text-2xl font-bold text-yellow-700">{quotationsCreated}</p>
                </button>
                <button className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-left" onClick={() => setFilterStage("Order Created")}>
                    <p className="text-sm text-gray-600">Orders Created</p>
                    <p className="text-2xl font-bold text-purple-700">{ordersCreated}</p>
                    <p className="text-xs text-gray-600 mt-1">Conversion: {conversionRatio.toFixed(1)}%</p>
                </button>
                <button className="bg-green-50 border border-green-200 rounded-lg p-4 text-left" onClick={() => setFilterStage("Order Processed")}>
                        ...
                        filteredLeads.map((lead) => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() =>
                                        router.push(`/user-dashboard/view-customer/${lead.customer_id}`)
                                    }
                                >
                                    ...
                                    <div className="mt-3 border-t pt-2 text-xs text-gray-700 space-y-1">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="ml-2 text-right">
                                                {lead.quote_number
                                                    ? `${lead.quote_number} (${lead.quote_date
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className="ml-2 text-right">
                                                {lead.order_id
                                                    ? `${lead.invoice_number || lead.order_id} (${lead.invoice_date
                                                        ? new Date(lead.invoice_date).toLocaleDateString()
                                                        : "-"})`
                                                    : "-"}
                                            </span>
                                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-4 py-3 text-sm max-w-xs">
                                                ...
                                                <div className="text-gray-500 text-xs break-all">{lead.email}</div>
                                            </td>
