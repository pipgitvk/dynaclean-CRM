      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <select {...register("assigned_to")}
            className="w-full border border-gray-300 rounded px-3 py-2" disabled={loadingUsers}>
            ...
            <table className="w-full text-sm text-left border-collapse table-auto">
              <thead className="bg-gray-800 text-white sticky top-0 z-10 shadow-md">
                <tr>
                  {TABLE_HEADERS.map((header) => (
                    <th key={header} className={`p-3 border-b border-gray-700 text-nowrap ${header === "Spec" ? "min-w-[200px] resize-x overflow-hidden" : ""}`}>
                      {header}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {loading ? (
                    ...
                    <tr key={i} className="odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                      ...
                      <td className="p-3 border-b border-gray-200 hidden md:table-cell">
                        <div className="space-y-1 text-xs">
                          ...
                          {r.invoice_file && <a href={`/uploads/${r.invoice_file}`} target="_blank" className="text-blue-600 hover:text-blue-800 underline">View File</a>}
                        </div>
                      </td>
                      <td className="p-3 border-b border-gray-200 space-y-1">
                        {r.report_file ? (
                          r.report_file.split(",").map((f, idx) => (
                            <div key={idx}><a href={`/uploads/${f}`} target="_blank" className="text-blue-600 hover:text-blue-800 underline">Report {idx + 1}</a></div>
                          ))
                        ) : <div className="text-gray-400">---</div>}
                      </td>
                      <td className="p-3 border-b border-gray-200 space-y-1">
                  ...
                  <tr><td colSpan={12} className="text-center p-4 text-gray-500">No matching records found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
