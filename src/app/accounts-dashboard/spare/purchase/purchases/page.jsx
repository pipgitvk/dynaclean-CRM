...
function EditCustomerModal({ open, onClose, record, onSaved }) {
  ...
  useEffect(() => {
    if (!customerIdSearch.trim()) { setCustomerSuggestions([]); return; }
    let cancelled = false;
    setLoadingCustomers(true);
    fetch(`/api/customers/list?search=${encodeURIComponent(customerIdSearch)}`)
      .then((r) => r.json())
      .then((data) => { if (cancelled) return; setCustomerSuggestions(Array.isArray(data) ? data : []); })
      .catch(() => { if (!cancelled) toast.error("Failed to search customers"); })
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            />
            {loadingCustomers && <div className="absolute right-3 top-9 text-xs text-gray-500">Loading...</div>}
            {customerSuggestions.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-white border rounded shadow-lg max-h-60 overflow-y-auto">
                {customerSuggestions.map((customer) => (
                  <div key={customer.customer_id} className="p-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleCustomerSelect(customer)}>
                    <div className="font-medium">{customer.first_name} {customer.last_name || ''}</div>
                    <div className="text-xs text-gray-500">ID: {customer.customer_id} | {customer.phone || 'No Phone'} | {customer.company || 'No Company'}</div>
                  </div>
                ))}
              </div>
            )}
