  ...
  const handleBlur = (index, field, value) => {
    ...
    if (field === "rate") {
      ...
      if (newValue < lastNegPrice) {
        alert(`Rate cannot be lower than the Last Negotiation Price: ₹${lastNegPrice}`);
  ...
  return (
    <div className="overflow-x-auto border mt-4 rounded">
      <table className="min-w-[1000px] w-full text-sm text-left border">
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx} className="border-t">
              ...
              <td className="border px-2 py-2">
                {item.imageUrl ? (
                  <Image src={item.imageUrl} alt="Product" width={40} height={40} className="rounded object-cover" unoptimized />
                ) : "-"}
              </td>
              ...
              <td className="border px-2 py-2">
                <div className="relative">
                  <input
                    ...
                    readOnly={isEditMode}
                  />
                  {!isEditMode && activeRowIndex === idx && productSuggestions.length > 0 && (
              ...
              <td className="border px-2 py-2">
                <input
                  ...
                  onChange={(e) => { if (!isEditMode) handleChange(idx, "quantity", e.target.value); }}
                  className={`border p-1 w-16 text-xs rounded ${isEditMode ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  min="0" step="0.01" readOnly={isEditMode}
                />
              </td>
              ...
              <td className="border px-2 py-2">
                <input
                  type="number"
                  value={item.rate ?? 0}
                  onChange={(e) => { if (!isEditMode) handleChange(idx, "rate", e.target.value); }}
                  onBlur={(e) => { if (!isEditMode) handleBlur(idx, "rate", e.target.value); }}
                  className={`border p-1 w-24 text-xs rounded ${isEditMode ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  min="0" step="0.01" readOnly={isEditMode}
                />
              </td>
              <td className="border px-2 py-2">
                <input
                  ...
                  onChange={(e) => { if (!isEditMode) handleChange(idx, "discount_percent", e.target.value); }}
                  className={`border p-1 w-16 text-xs rounded ${isEditMode ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  min="0" max="100" step="0.01" readOnly={isEditMode}
                />
              </td>
              <td className="border px-2 py-2">
                <input
                  ...
                  onChange={(e) => { if (!isEditMode) handleChange(idx, "discount_amount", e.target.value); }}
                  className={`border p-1 w-20 text-xs rounded ${isEditMode ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  min="0" step="0.01" readOnly={isEditMode}
                />
              </td>
              <td className="border px-2 py-2">₹ {(item.taxable_value || 0).toFixed(2)}</td>
          ...
          <tr className="font-semibold bg-gray-100">
            ...
            <td className="border px-2 py-2"></td>
            <td className="border px-2 py-2"></td>
            <td className="border px-2 py-2">₹ {totals.totalDiscount.toFixed(2)}</td>
      ...
      <div className="mt-3 text-center">
        {!isEditMode && (
          <button type="button" onClick={addRow} className="bg-green-600 text-white px-4 py-1 rounded hover:bg-green-700 text-sm">
            + Add Product
          </button>
        )}
      </div>
