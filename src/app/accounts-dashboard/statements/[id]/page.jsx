...
export default async function StatementDetailPage({ params }) {
  ...
  if (!statement) {
    return (
      <div className="p-6 text-center text-red-600">Statement not found</div>
    );
  }

  const expense = statement.client_expense_id
    ? await getClientExpenseById(statement.client_expense_id)
    : null;
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6">
      ...
      {expense ? (
        <div className="bg-white border rounded-lg overflow-hidden shadow-sm">
          <h3 className="text-lg font-bold text-gray-800 p-4 border-b bg-gray-50">
        ...
        <div className="p-6 text-center text-gray-500 bg-gray-50 rounded-lg">
          No expense linked to this statement
        </div>
      )}

      <div className="flex flex-col sm:flex-row gap-4 justify-between mt-10">
        <Link
          href="/accounts-dashboard/statements"
        ...
        {expense && (
          <Link
            href={`/accounts-dashboard/client-expenses/edit/${expense.id}?from=statements`}
