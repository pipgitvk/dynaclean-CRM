      ...
      {expense && (
        <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div className="p-4 border-t bg-gray-50 flex flex-wrap gap-3">
            <Link
              href={`/accounts-dashboard/client-expenses/${expense.id}?from=statements`}
            ...
            {statements.length > 0 ? (
              statements.map((s) => (
                <Link
                  key={s.id}
                  href={`/accounts-dashboard/statements/${s.id}`}
              ...
              <Link
                href={`/accounts-dashboard/statements/add?expense_id=${expense.id}&amount=${expense.amount ?? ""}`}
