  ...
  try {
    ...
    if (action === "assign") {
      ...
      if (Assigned_Date !== undefined) {
        ...
        params.push(new Date(Assigned_Date));
      }
      if (notes !== undefined) {
        fields.push("notes = ?");
        params.push(notes);
      ...
      for (const key of allowedFields) {
        if (Object.prototype.hasOwnProperty.call(data, key)) {
          ...
          if (
            ["checklist", "technical_specs", "login_credentials"].includes(key)
          ) {
      ...
      await conn.execute(
        `UPDATE assets SET ${sets.join(", ")} WHERE asset_id = ?`,
        params,
      );
      // await conn.end();
      return NextResponse.json({
