...
export async function POST(req) {
  try {
    ...
    if (files.cheque_upload) {
      uploadedPaths.cheque_upload = await saveFileLocally(
        getFile(files.cheque_upload),
        "cheques"
      );
    }
    if (files.signature_upload) {
      uploadedPaths.signature_upload = await saveFileLocally(
        getFile(files.signature_upload),
        "signatures"
      );
    }
    if (files.dd_upload) {
      uploadedPaths.dd_upload = await saveFileLocally(
        getFile(files.dd_upload),
        "issued-dds"
      );
    }
    if (files.bg_format_upload) {
      uploadedPaths.bg_format_upload = await saveFileLocally(
        getFile(files.bg_format_upload),
        "bg-formats"
      );
    }
    if (files.original_bg_upload) {
      uploadedPaths.original_bg_upload = await saveFileLocally(
        getFile(files.original_bg_upload),
        "original-bgs"
      );
    }
    if (files.docs_upload) {
      uploadedPaths.docs_upload = await saveFileLocally(
