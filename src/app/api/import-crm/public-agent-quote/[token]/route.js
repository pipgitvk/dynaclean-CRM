...
export async function GET(_request, { params }) {
  try {
    ...
    const [rows] = await db.query(
      `SELECT agent_name, company_name, agent_quote_submitted_at
    ...
    const alreadySubmitted = row.agent_quote_submitted_at != null;
    const displayName =
    ...
    console.error("public-agent-quote GET:", error);
    return NextResponse.json(
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const [locked] = await connection.execute(
      `SELECT id, agent_quote_submitted_at
       FROM import_crm_agents
       WHERE quote_link_token = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    const agent = locked?.[0];
    if (!agent) {
      await connection.rollback();
      return NextResponse.json({ message: "Link not found" }, { status: 404 });
    }
    if (agent.agent_quote_submitted_at != null) {
      await connection.rollback();
      return NextResponse.json(
    ...
    const crmAgentId = agent.id;
    const [result] = await connection.execute(
      `INSERT INTO import_crm_agent_quotations (
        crm_agent_id, shipment_id, agent_id,
    ...
    await connection.execute(
      `UPDATE import_crm_agents
       SET agent_quote_submitted_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [crmAgentId],
    );
    ...
    console.error("public-agent-quote POST:", error);
    return NextResponse.json(
      { message: "Internal server error." },
      { status: 500 },
    );
