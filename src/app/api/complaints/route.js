...
export async function POST(req) {
  try {
    ...
    console.log("complaint_summary:", complaint_summary);
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Missing 1 required form fields." },
        { status: 400 },
      );
    }
    ...
    const toEmails = [recipientEmail, siteEmail, assignedEmail].filter(
      (email) =>
        email &&
        typeof email === "string" &&
        /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),
    ...
    if (toEmails.length === 0) {
      ...
      return NextResponse.json({
        success: true,
        message:
          "Complaint added, but no email sent due to invalid recipient addresses.",
      });
    }
  ...
  try {
    ...
    const [rows] = await conn.execute(
      `(SELECT username FROM rep_list 
        WHERE userRole IN ('SERVICE ENGINEER', 'SERVICE HEAD', 'SERVICE SUPPORT ') 
