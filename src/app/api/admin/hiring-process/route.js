...
export async function GET(req) {
  try {
    ...
    if (createdByTrimmed) {
      distSql += ` AND LOWER(TRIM(created_by_username)) = LOWER(TRIM(?))`;
    }
    ...
    let hrSql = `SELECT DISTINCT TRIM(created_by_username) AS u FROM hr_hiring_entries
      WHERE TRIM(COALESCE(created_by_username, '')) != ''${ymClause}${modeClause}${designationClause} ORDER BY u`;
    ...
    if (createdByTrimmed != null) {
      sql += ` AND LOWER(TRIM(e.created_by_username)) = LOWER(TRIM(?))`;
...
export async function DELETE(req) {
  try {
    ...
    const conn = await getDbConnection();
    const [r] = await conn.execute(`DELETE FROM hr_hiring_entries WHERE id = ?`, [id]);
    if (!r.affectedRows) {
      return NextResponse.json({ success: false, error: "Record not found." }, { status: 404 });
    }
