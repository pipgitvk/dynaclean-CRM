...
export async function PUT(req, { params }) {
  ...
  if (!serialNumber) {
    return NextResponse.json({ error: 'Serial number for update is required.' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!product_name || !new_serial_number || !warranty_period || !customer_name || !email || !contact || !invoice_number || !invoice_date) {
    ...
    const existingReportFiles = existingRows[0]?.report_file ? existingRows[0].report_file.split(',').filter(Boolean) : [];
    ...
    const combinedReportFiles = [...new Set([...existingReportFiles, ...newReportFiles])];
    ...
    return NextResponse.json({ success: true, message: 'Product warranty updated successfully!' });

  } catch (error) {
    console.error('Error updating product warranty:', error);
    return NextResponse.json({ error: 'Failed to update product warranty.', details: error.message }, { status: 500 });
  } finally {
    console.log(`[GET] DB connection closed`);
  }
}