...
export async function createProspect(formData) {
  ...
  if (!payload || !canAccessProspectsRole(payload.role)) {
    redirect("/director-dashboard/prospects?error=unauthorized");
  }
  ...
  if (!customer_id || !model) {
    redirect("/director-dashboard/prospects/new?error=required");
  }

  const qtyParsed = parseInt(String(qtyRaw ?? "").trim(), 10);
  if (!Number.isFinite(qtyParsed) || qtyParsed < 1) {
    redirect("/director-dashboard/prospects/new?error=qty");
  }
  const qty = qtyParsed;
  ...
  if (!createCommitCheck.ok) {
    redirect("/director-dashboard/prospects/new?error=commitment_past");
  }
  ...
  if (!mayUse) {
    redirect("/director-dashboard/prospects/new?error=forbidden_customer");
  }
  ...
  redirect("/director-dashboard/prospects");
}

export async function createProspectManual(formData) {
  const payload = await getSessionPayload();
  if (!payload || !canAccessProspectsRole(payload.role)) {
    redirect("/director-dashboard/prospects/add-manual?error=unauthorized");
  }
  ...
  if (!customer_id || !model) {
    redirect("/director-dashboard/prospects/add-manual?error=required");
  }

  const qtyParsed = parseInt(String(qtyRaw ?? "").trim(), 10);
  if (!Number.isFinite(qtyParsed) || qtyParsed < 1) {
    redirect("/director-dashboard/prospects/add-manual?error=qty");
  }
  const qty = qtyParsed;
  const amount = parseFloat(String(amountRaw).replace(/,/g, "")) || 0;
  const commitment_date = commitmentRaw ? commitmentRaw : null;
  ...
  if (!createCommitCheck.ok) {
    redirect("/director-dashboard/prospects/add-manual?error=commitment_past");
  }
  ...
  const mayUse = await userMayUseCustomerForProspect(conn, customer_id, payload);
  if (!mayUse) {
    redirect("/director-dashboard/prospects/add-manual?error=forbidden_customer");
  }
  ...
  revalidatePath("/director-dashboard/prospects");
  redirect("/director-dashboard/prospects");
}
...
export async function updateProspect(formData) {
  ...
  if (!Number.isFinite(idNum) || idNum < 1) {
    redirect("/director-dashboard/prospects?error=required");
  }
  ...
  if (!customer_id || !model) {
    redirect(
      `/director-dashboard/prospects/${idNum}/edit?error=required`,
    );
  }

  const qtyParsed = parseInt(String(qtyRaw ?? "").trim(), 10);
  if (!Number.isFinite(qtyParsed) || qtyParsed < 1) {
    redirect(`/director-dashboard/prospects/${idNum}/edit?error=qty`);
  }
  const qty = qtyParsed;
  const amount = parseFloat(String(amountRaw).replace(/,/g, "")) || 0;
  const commitment_date = commitmentRaw ? commitmentRaw : null;
  ...
  if (!updateCommitCheck.ok) {
    redirect(`/director-dashboard/prospects/${idNum}/edit?error=commitment_past`);
  }
  if (!canFinalSubmitWithCommitment(commitmentRaw, todayIst)) {
    redirect(`/director-dashboard/prospects/${idNum}/edit?error=final_deadline`);
  }
  ...
  if (admin) {
    ...
    if (!user) {
      redirect(`/director-dashboard/prospects/${idNum}/edit?error=unauthorized`);
    }
    [result] = await conn.execute(
      `UPDATE prospects SET model = ?, qty = ?, amount = ?, commitment_date = ?, notes = ?, finalized_at = CURRENT_TIMESTAMP
  ...
  if (affected < 1) {
    ...
    if (againRow?.finalized_at != null) {
      redirect(`/director-dashboard/prospects/${idNum}/edit?error=locked`);
    }
    redirect(`/director-dashboard/prospects/${idNum}/edit?error=forbidden`);
  }
...
export async function createProspectsBulk(formData) {
  const payload = await getSessionPayload();
  if (!payload || !canAccessProspectsRole(payload.role)) {
    redirect("/director-dashboard/prospects/new?error=unauthorized");
  }
  ...
  function redirectCustomers(suffix) {
    ...
    redirect(
      `/director-dashboard/prospects/new?${suffix}&customers=${encodeURIComponent(ids.join(","))}${quoteQs}`,
    );
  }
  ...
  try {
    ...
    const quoteQs = formQuoteNumber
      ...
      : "";
    redirect(
      `/director-dashboard/prospects/new?error=db&customers=${encodeURIComponent(ids)}${quoteQs}`,
    );
  } finally {
    connection.release();
  }
