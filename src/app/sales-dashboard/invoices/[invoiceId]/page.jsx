...
async function getInvoiceWithItems(invoiceNumber) {
  ...
  // Collect related invoice IDs (parent + children) for waterfall allocation
  let relatedInvoices = [{ id: invoice.id, number: invoice.invoice_number }];
  if (invoice.parent_id) {
    const [parent] = await conn.execute(
      "SELECT id, invoice_number FROM invoices WHERE id = ? LIMIT 1",
      [invoice.parent_id]
    );
  ...
  relatedInvoices = relatedInvoices.filter(inv => {
    if (seenRelated.has(inv.id)) return false;
    seenRelated.add(inv.id);
    return true;
  });

  // Fetch grand_totals for all related invoices
  const relatedGrandTotals = {};
  for (const inv of relatedInvoices) {
    if (inv.id === invoice.id) {
      relatedGrandTotals[inv.id] = Number(invoice.grand_total) || 0;
    } else {
      const [[rel]] = await conn.execute("SELECT grand_total FROM invoices WHERE id = ? LIMIT 1", [inv.id]);
      relatedGrandTotals[inv.id] = Number(rel?.grand_total) || 0;
    }
  }

  // Fetch linked statements for all related invoices
  ...
  linkedStatements = linkedStatements.filter(s => {
    if (seenStmts.has(s.id)) return false;
    seenStmts.add(s.id);
    return true;
  });

  // Waterfall allocation across related invoices
  ...
  for (const stmt of linkedStatements) {
    ...
    const orderedIds = tokens
      .map(t => { const m = String(t).toUpperCase().match(/^IP(\d+)$/); return m ? parseInt(m[1]) : null; })
      .filter(id => id !== null && relatedGrandTotals[id] !== undefined);
