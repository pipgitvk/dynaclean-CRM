...
export default function QuotationSalesEditForm({ quoteId, hasOrder = false }) {
  ...
  const taxSummary = useMemo(() => {
    ...
    const isInterstate = (() => {
      ...
      const stateCode =
        parseCodeFromDisplay(form.state_name) ||
        Object.entries(STATE_CODE_TO_NAME).find(
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto px-4 text-gray-800">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {quoteNumber} {form.company ? `— ${form.company}` : ""}
          </p>
        </div>
        <button
          type="button"
      ...
      {hasOrder ? (
        <p className="text-red-600 text-sm font-medium bg-red-50 border border-red-200 p-3 rounded">
          ⚠️ This quotation is linked to an order and cannot be edited.
        </p>
      ) : null}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <input
          ...
          value={form.gstin_no}
          onChange={(e) =>
            setForm({ ...form, gstin_no: e.target.value.toUpperCase() })
          }
          disabled={hasOrder}
        />
        {getStateFromGSTIN(form.gstin_no?.trim()) ? (
          ...
          <div className="relative">
            <input
              ...
              disabled={hasOrder}
            />
            {showStateSuggestions && stateSuggestions.length > 0 && !hasOrder && (
      ...
      <div className="border p-4 rounded bg-gray-50">
        ...
        <textarea
          ...
          className="w-full text-sm border rounded-md p-3 min-h-[140px] bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
          disabled={hasOrder}
        />
      </div>
      ...
      <div className="flex gap-4 justify-end pb-8">
        ...
        >
          {isSubmitting ? "Saving..." : hasChanges ? "Save as New Quotation" : "Save"}
        </button>
      </div>
    </form>
  );
