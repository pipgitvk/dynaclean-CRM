...
function buildByCreatorHref(name, sp) {
  const base = `/sales-dashboard/prospects/by-creator/${encodeURIComponent(name)}`;
...
export default function ProspectsListCard({
  ...
  useEffect(() => {
    ...
    const base = lockedCreatorName
      ? `/sales-dashboard/prospects/by-creator/${encodeURIComponent(lockedCreatorName)}`
      : "/sales-dashboard/prospects";
    const qs = p.toString();
  ...
  const navigateToAddFromSuggestion = useCallback(
    (s) => {
      ...
      router.push(`/sales-dashboard/prospects/new?customers=${q}${quoteQs}`);
    },
    [router],
  );
    ...
    return (
      <>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <Link
            href="/sales-dashboard/prospects/add-manual"
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-2 py-2.5 sm:px-4 sm:py-3">
                      <div className="flex items-center gap-1 sm:gap-1.5">
                        {canEdit ? (
                          <Link
                            href={`/sales-dashboard/prospects/${row.id}/edit`}
