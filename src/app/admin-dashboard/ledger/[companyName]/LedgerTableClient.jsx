...
export default function LedgerTableClient({ rows: initialRows, companyName, customerId }) {
  ...
  const minDate = useMemo(() => {
    if (!initialRows || initialRows.length === 0) return dayjs().startOf("year").format("YYYY-MM-DD");
    const dates = initialRows.map((r) => String(r.entry_date).slice(0, 10)).sort();
    return dates[0];
  }, [initialRows]);

  const maxDate = useMemo(() => {
    if (!initialRows || initialRows.length === 0) return dayjs().format("YYYY-MM-DD");
    const dates = initialRows.map((r) => String(r.entry_date).slice(0, 10)).sort();
  ...
  const filtered = useMemo(() => {
    ...
    return data;
  }, [initialRows, sortCol, sortDir, dateFrom, dateTo]);
  ...
  const handleDownloadExcel = () => {
    if (filtered.length === 0) { toast.error("No entries to download"); return; }

    const csvLines = [];
    csvLines.push(`"${companySettings.company_name}"`);
    csvLines.push(`"${companySettings.company_address_line1}"`);
    csvLines.push(`"${companySettings.company_address_line2}"`);
    csvLines.push(`"E-Mail: ${companySettings.company_email}"`);
    csvLines.push("");
    csvLines.push(`"Ledger Account"`);
    csvLines.push(`"${companyName}"`);
    if (customerId) csvLines.push(`"Customer ID: ${customerId}"`);
    csvLines.push("");
    csvLines.push(`"Period: ${dayjs(dateFrom).format("DD-MMM-YY")} to ${dayjs(dateTo).format("DD-MMM-YY")}"`);
    csvLines.push("");
    csvLines.push(["Date", "Particulars", "Vch Type", "Vch No", "Debit (₹)", "Credit (₹)"].map((h) => `"${h}"`).join(","));
    ...
    csvLines.push("");
    csvLines.push(["", '"Total"', "", "", `"${totals.debit.toFixed(2)}"`, `"${totals.credit.toFixed(2)}"`].join(","));

    if (totals.debit !== totals.credit) {
      csvLines.push([
        "", '"Balance c/d"', "", "",
        totals.credit > totals.debit ? `"${(totals.credit - totals.debit).toFixed(2)}"` : `""`,
        totals.debit  > totals.credit ? `"${(totals.debit  - totals.credit).toFixed(2)}"` : `""`,
      ].join(","));
    }
    ...
    link.setAttribute("download", `ledger_${companyName.replace(/\s+/g, "_")}_${dayjs().format("DD-MM-YYYY")}.csv`);
  ...
  const handleDownloadPDF = () => {
    ...
    const addHeader = (pNo = 1) => {
      ...
      y += 22;
      center(safe(companyName), 13, true, 13);
      center("Ledger Account", 10, true, 11);
      y += 3;
      center(dateRangeStr, 10, true, 16);
      right(`Page ${pNo}`, tableRight, y, 9, true);
      y += 8;
      drawTableHeader();
    };
    ...
    filtered.forEach((r) => {
      ...
      const wrappedParticulars = pdf.splitTextToSize(safe(r.particulars), 165);
    ...
    y += 12;
    if (y > pageH - 90) { pdf.addPage(); pageNo += 1; addHeader(pageNo); }
    ...
    pdf.save(`ledger_${companyName.replace(/\s+/g, "_")}_${dayjs().format("DD-MM-YYYY")}.pdf`);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          { label: "Net Balance",   value: Math.abs(totals.balance),
            ...
            <tfoot>
              {totals.debit !== totals.credit && (
                <tr className="bg-green-50 border-t border-gray-200">
                  ...
                  <td className="px-4 py-2 text-xs text-gray-500 italic">Balance c/d</td>
                  <td /><td />
                  <td className="px-4 py-2 text-right font-mono text-red-500 text-xs">
                    {totals.credit > totals.debit ? `₹${fmt(totals.credit - totals.debit)}` : "—"}
                  </td>
                  <td className="px-4 py-2 text-right font-mono text-green-600 font-semibold bg-green-100">
                    {totals.debit > totals.credit ? `₹${fmt(totals.debit - totals.credit)}` : "—"}
                  </td>
                </tr>
              )}
