...
export default function OtherIncomeTable({ rows, role, isAdmin }) {
  ...
  const users = Array.from(new Set(rows?.map((row) => row.username))).filter(Boolean).sort();

  // Filter rows based on search, status, date range, and user (for admin)
  const filteredRows = (rows || []).filter((row) => {
    ...
    const matchesUser = !selectedUser || row.username === selectedUser;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow space-y-4">
        ...
        <div className={`grid ${isAdmin ? "lg:grid-cols-5" : "lg:grid-cols-4"} gap-4`}>
          <div>
            ...
            {sortedRows.length > 0 ? (
              sortedRows.map((row, idx) => (
                <tr key={row.id || idx} className="border-b hover:bg-gray-50">
                  {isAdmin && (
                    <td className="px-4 py-3 text-gray-700 font-medium text-sm">{row.username}</td>
                  )}
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex justify-center gap-2">
                      <Link
                        href={`/admin-dashboard/other-income/${row.id}`}
                      ...
                      {["Pending"].includes(row.approval_status) && (
                        <Link
                          href={`/admin-dashboard/other-income/edit/${row.id}`}
