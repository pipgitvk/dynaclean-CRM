...
import OrderTable from "@/app/admin-dashboard/order/OrderTable";
...
export default async function CustomerOrdersPage({ params }) {
  ...
  let sql = `SELECT 
            ...
            WHERE no.customer_id = ? AND no.created_by = ?
            GROUP BY no.order_id ORDER BY no.created_at DESC`;

  const [orders] = await conn.execute(sql, [customerId, username]);
    ...
    <div className="mx-auto p-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href={`/admin-dashboard/order/new?customerId=${customerId}`}
